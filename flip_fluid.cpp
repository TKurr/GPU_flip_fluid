                         ...
                         int numSubSteps)
{
    ...
    for (int step = 0; step < numSubSteps; ++step) {
        ...
        frameMs[T1_INTEGRATE] += std::chrono::duration<double, std::milli>(std::chrono::steady_clock::now() - t0).count();

        t0 = std::chrono::steady_clock::now();
        if (separateParticles) pushParticlesApart(numParticleIters);
        frameMs[T2_PUSH_APART] += std::chrono::duration<double, std::milli>(std::chrono::steady_clock::now() - t0).count();

        t0 = std::chrono::steady_clock::now();
        handleParticleCollisions(obstacleX, obstacleY, obstacleRadius, obstacleVelX, obstacleVelY);
        frameMs[T3_COLLISIONS] += std::chrono::duration<double, std::milli>(std::chrono::steady_clock::now() - t0).count();

        t0 = std::chrono::steady_clock::now();
        transferVelocities(true);
        frameMs[T4_P2G] += std::chrono::duration<double, std::milli>(std::chrono::steady_clock::now() - t0).count();

        t0 = std::chrono::steady_clock::now();
        updateParticleDensity();
        if (particleRestDensity == 0.0f) particleRestDensity = computeRestDensity();
        frameMs[T5_DENSITY] += std::chrono::duration<double, std::milli>(std::chrono::steady_clock::now() - t0).count();

        t0 = std::chrono::steady_clock::now();
        solveIncompressibility(numPressureIters, sdt, overRelaxation, compensateDrift);
        frameMs[T6_PRESSURE] += std::chrono::duration<double, std::milli>(std::chrono::steady_clock::now() - t0).count();

        t0 = std::chrono::steady_clock::now();
        transferVelocities(false, flipRatio);
        frameMs[T7_G2P] += std::chrono::duration<double, std::milli>(std::chrono::steady_clock::now() - t0).count();
    }
    
    auto t0 = std::chrono::steady_clock::now();
    updateParticleColors();
    updateCellColors();
    frameMs[T8_COLORS] += std::chrono::duration<double, std::milli>(std::chrono::steady_clock::now() - t0).count();

    for (int i = 0; i < T8_COLORS + 1; ++i) {
        accumMs[i] += frameMs[i];
    }
    accumFrames++;
}

void FlipFluid::resetTiming() {
    for (int i = 0; i < NUM_TIMING_STAGES; i++) accumMs[i] = 0.0;
    accumFrames = 0;
}

void FlipFluid::printTiming() {
